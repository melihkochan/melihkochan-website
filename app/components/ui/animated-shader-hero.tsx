...
void main(){gl_Position=position;}`;
  ...
  const resize = () => {
    ...
    if (rendererRef.current) {
      ...
      (rendererRef.current as any).updateViewport();
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Silently fail - content will still be visible
    }
  }, []);
...
const Hero: React.FC<HeroProps> = ({
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden bg-gradient-to-br from-black via-gray-900 to-black ${className}`}>
      <canvas
        ref={canvasRef}
        className="absolute inset-0 w-full h-full object-contain touch-none pointer-events-none"
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen text-white px-4 py-20">
        ...
        <div className="text-center max-w-5xl mx-auto px-4">
          ...
          <div>
            <div className="text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-white flex items-center justify-center gap-4 flex-wrap">
              <span className="text-white">Hi I&apos;m {headline.line1}</span>
            </div>
            {subtitle && (
              <div className="text-lg md:text-xl lg:text-2xl font-normal text-white mt-12">
                {subtitle}
              </div>
            )}
          </div>
          ...
          {description && (
            <div className="max-w-3xl mx-auto mt-20" style={{ marginBottom: '20px' }}>
